...
export class RaftNode extends EventEmitter {
  ...
  __initialize() {
    this.on(MESSAGE.TERM_CHANGE, (packet: Packet) => {
    ...
    this.on(MESSAGE.STATE_CHANGE, (packet: Packet) => {});

    this.on(MESSAGE.DATA, (packet: Packet) => {});
  }

  resetElectionTimeout(): number {
    return Math.floor(
      Math.random() * (ELECTION_TIMER - HEARTBEAT_INTERVAL) + HEARTBEAT_INTERVAL
    );
  }
  ...
  async startElection(): Promise<void> {
    ...
    console.log(
      `Node ${this.id} is starting an election for term ${this.term}`
    );
